...
export default function HomePage() {
  ...
  const mainContentHeight = "calc(100vh - 64px)"; // Example: 64px for header height

  return (
    <div
      className="bg-gradient-to-br from-background via-surface to-background text-text-primary flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-8"
      style={{ minHeight: mainContentHeight }} // Ensure it takes at least the remaining viewport height
    >
      <div className="max-w-3xl"> {/* Limiting width for better readability */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-tight">
          Welcome to <span className="text-primary">{OJ_NAME}</span>!
        </h1>
        ...
        <div className="mt-8 md:mt-10 flex flex-col sm:flex-row justify-center items-center gap-4 md:gap-6">
          <Link
            to="/problems/all"
            className="w-full sm:w-auto bg-primary text-white font-semibold px-8 py-3 md:px-10 md:py-4 rounded-lg shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-surface focus:ring-primary transition-transform transform hover:scale-105 text-base md:text-lg"
          >
            Explore Problems
          </Link>
          {!currentUser && (
